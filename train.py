...
def parser_options():
    ...
    parser.add_argument('--path_opt', default='/home/mcx/RS/KAMCL-main/option/RSICD/RSICD.yaml', type=str,
...
def main(options):
    # choose model
    if options['model']['name'] == "KAMCL":
        from layers import KAMCL as models
    else:
    ...
    for epoch in range(start_epoch, options['optim']['epochs']):
        ...
        # train for one epochW
        engine.train(train_loader, model, optimizer, epoch, opt=options)

        # evaluate on validation set
        if epoch % options['logs']['eval_step'] == 0:
            currentscore, all_scores = engine.validate(val_loader, model)
            is_best = currentscore > best_rsum
            if is_best:
                best_score = all_scores
            best_rsum = max(currentscore, best_rsum)
